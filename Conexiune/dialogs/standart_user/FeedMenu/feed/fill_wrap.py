from datetime import datetime

from aiogram_dialog import DialogManager
from sqlalchemy.ext.asyncio import AsyncSession

from dialogs.api import to_file_id, get_u_id
from dialogs.standart_user.FeedMenu.db_int import get_profiles, get_setts


async def slct20_handle_back(session: AsyncSession,
                             tg_id: int,
                             data: DialogManager.dialog_data,
                             offset=False,
                             ):
    #### GETTING SETTINGS
    if 'setts' not in data:
        data['setts'] = await get_setts(
            session,
            data['u_id'] if 'u_id' in data else await get_u_id(session, tg_id, data)
        )
    #### OFFSET+SLCT
    if offset:
        p_list: list = await get_profiles(
            session,
            sex=data['setts']['sex'],  # todo: rewrite get_profiles(-sex +fs)
            offset=data['offset']
        )  # TODO: handle p nothing
        data['offset'] += 1
    else:
        p_list: list = await get_profiles(
            session,
            sex=data['setts']['sex'],
        )  # TODO: handle p nothing
        data['offset'] = 1
    data['show_list'] = []  # CREATION | ANNULMENT
    for p in p_list:
        data['show_list'].append({
            'photo': p.photo, 'name': p.name,
            'age': p.age, 'sex': p.sex,
            'location': p.location, 'descr': p.descr
        })
    data['time_mark'] = datetime.now().timestamp()  # ANNULMENT
    #### POINTER
    return p_list[0]


async def slct_p_ram(data: DialogManager.dialog_data):
    try:
        p = data['profiles'][0]
        del data['profiles'][0]
        return p
    except:  # TODO: finish
        ...


async def wrap_front_dict(p):
    if p is dict:
        p_front = {
            'photo': await to_file_id(p['photo']),
            'name': p['name'],
            'age': p['age'],
            'sex': p['sex'],
            'location': p['location'],
            'descr': p['descr'],
        }
    else:
        p_front = {
            'photo': await to_file_id(p.photo),
            'name': p.name,
            'age': p.age,
            'sex': p.sex,
            'location': p.location,
            'descr': p.descr,
        }
    return p_front
